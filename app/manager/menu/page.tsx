...
function CreateMenuItemWizard({ isOpen, onClose, products, stocks, onCreateItem, onAddIngredient }: CreateMenuItemWizardProps) {
  ...
  // Calcul GPAO du coût des ingrédients
              ...
              <div className="menu-ingredients-list">
                ...
                {ingredients.length === 0 ? (
                  ...
                  <div className="menu-ingredients-items">
                    {ingredients.map(ing => {
                      // Calcul GPAO du coût de cet ingrédient
                      const stock = getProductStock(ing.product_id)
                      const stockUnitPrice = stock ? Number(stock.unit_price) : 0
                      const stockUnit = stock?.product?.unit || 'unités'
                      const ingredientCost = calculateIngredientCostGPAO(
                        ing.quantity,
                        ing.unit,
                      ...
                      return (
                        <div key={ing.product_id} className="menu-ingredient-item">
                          <div className="menu-ingredient-item-info">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-slate-500">
                                {stockUnitPrice > 0 ? `${stockUnitPrice.toFixed(2)}€/${stockUnit}` : 'Prix non défini'}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            <span className={`text-sm font-medium ${ingredientCost > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
...
function MenuItemDetail({ item, isOpen, onClose, onUpdatePrice, getSuggestedPrice, getQuickWins, stocks }: MenuItemDetailProps) {
            ...
            <div className="menu-detail-recipe">
              ...
              <div className="menu-detail-ingredients">
                {item.ingredients.map(ing => {
                  // Calcul GPAO du coût de cet ingrédient
                  const stock = stocks.find(s => s.product_id === ing.product_id)
                  const stockUnitPrice = stock ? Number(stock.unit_price) : 0
                  const stockUnit = stock?.product?.unit || 'unités'
                  const ingredientCost = calculateIngredientCostGPAO(
                    Number(ing.quantity),
                  ...
                  return (
                    <div key={ing.id} className="menu-detail-ingredient">
                      ...
                      <span className="menu-detail-ingredient-cost">{ingredientCost.toFixed(2)} €</span>
                    </div>
                  )
                })}
              </div>
...
export default function ManagerMenuPage() {
  ...
  // Récupérer les stocks pour les calculs GPAO
